...
`proxy-server.<type>.handoff_all_count`   Count of times all hand-off locations were utilized;
                                          only tracked if log_handoffs is set in the
                                          proxy-server config.
...
proxy-server controller responsible for the request: "Account", "Controller",
or "Object", or the string "SOS" if the request came from the `Swift Origin Server`_
middleware.
...
Metric Name                                   Description
--------------------------------------------  ----------------------------------------------------
`proxy-server.<type>.<verb>.<status>.timing`  Timing data for requests.  The <type> portion will
                                              be one of "Account", "Container", "Object", or "SOS".
                                              The <verb> portion is the HTTP verb for the request
                                              and will be one of "GET", "HEAD", "POST", "PUT",
                                              "DELETE", or "COPY".  The <status> portion is the
                                              numeric HTTP status code for the request (eg. "200"
                                              or "404")
